...
<div align="center">
    ...
    <button type="button" id="blockUnblock_user">Block/Unblock</button>
    ...
    <table border="1">
        <c:forEach items="${users}" var="user">
            <tr>
                ...
                <td><c:out value="${user.isNonBlocked}"/></td>
            </tr>
        </c:forEach>
    </table>
</div>
...
<script>
    ...
    $('#blockUnblock_user').
    click(function() {
        var data = {'toBlockUnblock' : []};
        $(":checked").each(function() {
            data['toBlockUnblock'].push($(this).val());
        });
        $.post("/blockedUnblocked", data, function(data, status) {
            window.location.reload();
        });
    });
</script>
